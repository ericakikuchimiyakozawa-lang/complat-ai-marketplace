...
## 解約クライアントの扱い
本リポは **公開** のため、「アクセス剥奪で更新を止める」運用はしない。解約時はクライアント側で
プラグインを無効化する（`/plugin` 管理画面でOFF、または案件フォルダの `.claude/settings.json` から
`enabledPlugins` を外す）。あわせてサポート停止・案件フォルダ回収など契約/運用面で締める。
